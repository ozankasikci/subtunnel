...
const DASHBOARD_URL = process.env.NEXT_PUBLIC_DASHBOARD_URL || "https://dashboard.subtunnel.dev";
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create account
      const res = await fetch(`${DASHBOARD_URL}/api/auth/signup`, {
        method: "POST",
      ...
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-accent/10 border border-accent/20 mb-3">
            <svg width="20" height="20" viewBox="0 0 16 16" fill="none" className="text-accent">
              <path d="M2 8h4l2-6 2 12 2-6h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold">Create an account</h1>
          <p className="text-muted text-sm mt-1">Get started with SubTunnel for free</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="flex items-center justify-center gap-2 bg-background border border-border rounded-lg px-3 py-2.5 text-sm hover:bg-card-hover transition-colors"
            >
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="block w-full text-center bg-accent hover:bg-accent-hover text-black font-medium rounded-lg py-2.5 text-sm transition-colors disabled:opacity-50"
            >
          ...
          <p className="text-xs text-muted text-center">
            ...
            <a href="#" className="text-accent hover:text-accent-hover">Terms</a> and{" "}
